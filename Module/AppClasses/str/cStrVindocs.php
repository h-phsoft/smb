...
class cStrVindocs {
  ...
  public static function getCount($vWhere = '') {
    $nCount = 0;
    $sSQL = 'SELECT count(*) nCnt FROM `str_vindocs`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
    $res = ph_Execute($sSQL);
  ...
  public static function getArray($vWhere = '', $vOrder = '', $nPage = 0, $nPageSize = 0) {
    ...
    $res = ph_Execute(cStrVindocs::getSelectStatement($vWhere, $vOrder, $vLimit));
    if ($res != '') {
      while (!$res->EOF) {
        $aArray[$nIdx] = cStrVindocs::getFields($res);
        $nIdx++;
        $res->MoveNext();
      }
      $res->Close();
  ...
  public static function getInstance($nId) {
    $cClass = new cStrVindocs();
    $res = ph_Execute(cStrVindocs::getSelectStatement('(`id`="' . $nId . '")'));
    if ($res != '') {
      if (!$res->EOF) {
        $cClass = cStrVindocs::getFields($res);
      }
      $res->Close();
    }
    return $cClass;
  ...
  public function delete() {
    $vSQL = 'DELETE FROM `str_vindocs` WHERE `id`="' . $this->Id . '"';
    $res = ph_Execute($vSQL);
    if ($res || $res === 0) {
