...
class cStrInMst {
  ...
  public static function getCount($vWhere = '') {
    $nCount = 0;
    $sSQL = 'SELECT count(*) nCnt FROM `str_vinmst`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
    $res = ph_Execute($sSQL);
  ...
  public static function getArray($vWhere = '', $vOrder = '', $nPage = 0, $nPageSize = 0) {
    ...
    $res = ph_Execute(cStrInMst::getSelectStatement($vWhere, $vOrder, $vLimit));
    if ($res != '') {
      while (!$res->EOF) {
        $aArray[$nIdx] = cStrInMst::getFields($res);
        $nIdx++;
        $res->MoveNext();
      }
      $res->Close();
  ...
  public static function getInstance($nId) {
    $cClass = new cStrInMst();
    $res = ph_Execute(cStrInMst::getSelectStatement('(`mst_id` = "' . $nId . '")'));
    if ($res != '') {
      if (!$res->EOF) {
        $cClass = cStrInMst::getFields($res);
      }
      $res->Close();
    }
    return $cClass;
  ...
  public function delete() {
    ...
    try {
      foreach ($aTrn as $oTrn) {
        ...
        $oStrItem->subtractQnt($oTrn->Qnt, $oTrn->Amt);
        $oTrn->delete();
      }
      $vSQL = 'DELETE FROM `str_inmst` WHERE `id`="' . $this->MstId . '"';
      $res = ph_Execute($vSQL);
      if ($res || $res === 0) {
