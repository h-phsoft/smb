...
class cStrVinmst {
  ...
  public static function getArray($vWhere = '', $vOrder = '', $nPage = 0, $nPageSize = 0) {
    ...
    $res = ph_Execute(cStrVinmst::getSelectStatement($vWhere, $vOrder, $vLimit));
    if ($res != '') {
      while (!$res->EOF) {
        $aArray[$nIdx] = cStrVinmst::getFields($res);
        $nIdx++;
        $res->MoveNext();
      }
      $res->Close();
  ...
  public static function getInstance($nId) {
    $cClass = new cStrVinmst();
    $res = ph_Execute(cStrVinmst::getSelectStatement('(`id`="' . $nId . '")'));
    if ($res != '') {
      if (!$res->EOF) {
        $cClass = cStrVinmst::getFields($res);
      }
      $res->Close();
    }
    return $cClass;
  ...
  public function delete() {
    $vSQL = 'DELETE FROM `str_vinmst` WHERE `id`="' . $this->Id . '"';
    $res = ph_Execute($vSQL);
    if ($res || $res === 0) {
