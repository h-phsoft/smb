...
class cStrVoutrn {
  ...
  public static function getCount($vWhere = '') {
    $nCount = 0;
    $sSQL = 'SELECT count(*) nCnt FROM `str_voutrn`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
    $res = ph_Execute($sSQL);
  ...
  public static function getArray($vWhere = '', $vOrder = '', $nPage = 0, $nPageSize = 0) {
    ...
    $res = ph_Execute(cStrVoutrn::getSelectStatement($vWhere, $vOrder, $vLimit));
    if ($res != '') {
      while (!$res->EOF) {
        $aArray[$nIdx] = cStrVoutrn::getFields($res);
        $nIdx++;
        $res->MoveNext();
      }
      $res->Close();
  ...
  public static function getInstance($nId) {
    $cClass = new cStrVoutrn();
    $res = ph_Execute(cStrVoutrn::getSelectStatement('(`id`="' . $nId . '")'));
    if ($res != '') {
      if (!$res->EOF) {
        $cClass = cStrVoutrn::getFields($res);
      }
      $res->Close();
    }
    return $cClass;
  ...
  public function delete() {
    $vSQL = 'DELETE FROM `str_voutrn` WHERE `id`="' . $this->Id . '"';
    $res = ph_Execute($vSQL);
    if ($res || $res === 0) {
