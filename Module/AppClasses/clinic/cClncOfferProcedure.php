...
class cClncOfferProcedure {
  ...
  public static function getSelectStatement($vWhere = '', $vOrder = '', $vLimit = '') {
    $sSQL = 'SELECT `id`, `offer_id`, `procedure_id`, `price`, `description`, `ins_user`, `ins_date`'
      . ', `upd_user`, `upd_date`'
      . ' FROM `clnc_offer_procedure`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
    if ($vOrder != '') {
  ...
  public static function getCount($vWhere = '') {
    $nCount = 0;
    $sSQL = 'SELECT count(*) nCnt FROM `clnc_offer_procedure`';
    if ($vWhere != '') {
      $sSQL .= ' WHERE (' . $vWhere . ') ';
    }
    $res = ph_Execute($sSQL);
  ...
  public static function getInstance($nId) {
    $cClass = new cClncOfferProcedure();
  ...
  public function save($nUId) {
    ...
    if ($this->Id == 0 || $this->Id == -999) {
      $vSQL = 'INSERT INTO `clnc_offer_procedure` ('
        . '  `offer_id`, `procedure_id`, `price`, `description`, `ins_user`' 
        . ') VALUES ('
        . '  "' . $this->OfferId . '"'
        . ', "' . $this->ProcedureId . '"'
        . ', "' . $this->Price . '"'
      ...
      $nId = $this->Id;
      $vSQL = 'UPDATE `clnc_offer_procedure` SET'
  ...
  public function delete() {
    $vSQL = 'DELETE FROM `clnc_offer_procedure` WHERE `id`="' . $this->Id . '"';
    $res = ph_Execute($vSQL);
    if ($res || $res === 0) {
