...
if (isset($oRest)) {
  ...
  $nPages = 0;
  $nCount = cClncProcedureCategory::getCount($vWhere);
  if (isset($size) && intval($size) > 0) {
    $nPages = ceil($nCount / $size);
  }
  $aList = cClncProcedureCategory::getArray($vWhere, $vOrder, $page, $size);
  $aData = array();
  $nIdx = 0;
  foreach ($aList as $element) {
    $aData[$nIdx] = array(
      'nId' => $element->Id,
      'nSpecId' => $element->SpecId,
      'vSpecName' => $element->oSpec->Name,
      'vName' => $element->Name,
