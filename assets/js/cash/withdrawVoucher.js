...
function openNew() {
  ...
  $('#fldBankId').val($('#fldBankId option:first').val());
...
function save() {
  ...
  if (form.checkValidity()) {
    if (parseFloat($('#fldCamt').val()) > 0 && $('#fldDate').val() >= PhSettings.WPSDate && $('#fldDate').val() <= PhSettings.WPEDate) {
      getAmount();
      PhUtility.doSave({
        'vOperation': 'cpy-Cash-Withdraw-Save',
        'nId': $('#fldId').val(),
        'nNum': $('#fldNum').val(),
        'dDate': $('#fldDate').val(),
...
function cellEditClick(e, cell) {
  ...
  $('#fldBankId').val(data.nBankId);
  $('#fldContName').val(data.vContName);
...
function cellDeleteClick() {
  if ($('#fldId').val() > 0) {
    PhUtility.doDelete($('#fldNum').val() + '  ' + $('#fldDate').val(), {
      'vOperation': 'cpy-Cash-Withdraw-Delete',
      'nId': $('#fldId').val()
    }, openNew);
  }
}
...
function refreshList() {
  ...
  table = getAjaxTabulator('cpy-Cash-Withdraw-List', aColumns);
}