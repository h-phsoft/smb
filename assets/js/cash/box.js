...
function openNew() {
  ...
  $('#fldCashId').val($('#fldCashId :first').val());
  $('#fldCostId').val('');
  $('#fldCostName').val('');
  $('#fldAccId').val('');
  $('#fldAccName').val('');
...
function save() {
  ...
  if (form.checkValidity()) {
    PhUtility.doSave({
      'vOperation': 'cpy-Cash-Box-Save',
      'nId': $('#fldId').val(),
      'nStatusId': $('#fldStatusId').val(),
      'nCashId': $('#fldCashId').val(),
...
function cellEditClick(e, cell) {
  ...
  $('#fldCashId').val(data.nCashId);
  $('#fldCashName').val(data.vCashName);
...
function cellDeleteClick(e, cell) {
  ...
  PhUtility.doDelete(data.vName, {
    'vOperation': 'cpy-Cash-Box-Delete',
    'nId': data.nId
  }, refreshALL);
}
...
function refreshList() {
  ...
  table = getAjaxTabulator('cpy-Cash-Box-List', aColumns);
}
