...
function save() {
  ...
  if (form.checkValidity()) {
    if (parseFloat($('#fldCamt').val()) > 0 && $('#fldDate').val() >= PhSettings.WPSDate && $('#fldDate').val() <= PhSettings.WPEDate) {
      getAmount();
      PhUtility.doSave({
        'vOperation': 'cpy-Cash-Payment-Save',
        'nId': $('#fldId').val(),
        'nNum': $('#fldNum').val(),
        'dDate': $('#fldDate').val(),
...
function cellDeleteClick() {
  PhUtility.doDelete($('#fldNum').val() + '  ' + $('#fldDate').val(), {
    'vOperation': 'cpy-Cash-Payment-Delete',
    'nId': $('#fldId').val()
  }, openNew);
}
...
function refreshList() {
  ...
  table = getAjaxTabulator('cpy-Cash-Payment-List', aColumns);
}