...
function openNew() {
  ...
  $('#fldPatientId').val(0);
  $('#fldPatientName').val('');
  $('#fldDoctorId').val($('#fldDoctorId :first').val());
  $('#fldMethodId').val($('#fldMethodId :first').val());
  $('#fldDate').val('');
  $('#fldAmt').val(0);
  $('#fldDescription').val('');
  $('#ph_Modal').modal('show');
...
function save() {
  ...
  if (form.checkValidity()) {
    PhUtility.doSave({
      'vOperation': 'cpy-Clinic-Finance-Payment-Save',
      'nId': $('#fldId').val(),
      // 'nClinicId': $('#fldClinicId').val(),
      'nClinicId': 1,
      'nPatientId': $('#fldPatientId').val(),
      'vPatientName': $('#fldPatientName').val(),
      'nDoctorId': $('#fldDoctorId').val(),
...
function cellEditClick(e, cell) {
  ...
  $('#fldDoctorId').val(data.nDoctorId);
...
function cellDeleteClick(e, cell) {
  ...
  PhUtility.doDelete(data.vName, {
    'vOperation': 'cpy-Clinic-Finance-Payment-Delete',
    'nId': data.nId
  }, refreshALL);
}
...
function refreshList() {
  ...
  aColumns[nIdx++] = {
    title: getLabel('Patient Number'),
    field: 'vPatientNumber',
    hozAlign: 'left',
    headerHozAlign: 'center',
    headerFilter: 'input',
    formatter: 'textarea'
  ...
  table = getAjaxTabulator('cpy-Clinic-Finance-Payment-List', aColumns);
}
