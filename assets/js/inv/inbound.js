/* global KTAppSettings, PhTabulatorLocale, PhSettings, KTUtil, swal, PhUtility, PhTable_WIDTH_VAIABLE */
...
function refreshList() {
  ...
  table = getAjaxTabulator("cpy-Warehouse-Inbound-ListInbounds", aColumns);
}
...
function cellEditClick(e, cell) {
  ...
  $('#fldCostId').val(data.CostId);
  $('#fldCostName').val(data.CostName);
  ...
  $.ajax({
    ...
    data: {
      ...
      "vOperation": "cpy-Warehouse-Inbound-GetInbound",
      "nId": data.Id
    },
...
function cellDeleteClick(e, cell) {
  var data = cell.getData();
  PhUtility.doDelete(getLabel('Inbound'), {
    'vOperation': "cpy-Warehouse-Inbound-InboundDelete",
    'nId': data.Id
  }, refreshList, true);
}
...
function initPhTable() {
  ...
  phTable = new PhTable('phTable', aColumns, [], {widthType: PhTable_WIDTH_VAIABLE});
}
...
function save() {
  ...
  if (form.checkValidity()) {
    ...
    PhUtility.doSave({
      'vOperation': "cpy-Warehouse-Inbound-InboundSave",
      'nId': $('#fldId').val(),
      'nDoc': $('#fldDoc').val(),
      'nStore': $('#fldStore').val(),
      'nAcc': $('#fldAccId').val(),
